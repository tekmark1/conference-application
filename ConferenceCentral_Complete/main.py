...
class CheckFeaturedSpeakerHandler(webapp2.RequestHandler):
    def post(self):
        ...
        # get websafeConferenceKey (parent) and speaker from taskqueue
        wsck = self.request.get('wsck')
        speaker = self.request.get('speaker')

        # call _checkFeaturedSpeaker from ConferenceAPI which updateds memcache
        ConferenceApi._checkFeaturedSpeaker(wsck, speaker)
        self.response.set_status(204)
